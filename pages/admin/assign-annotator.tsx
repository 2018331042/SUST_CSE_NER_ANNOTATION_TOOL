...
const AssignAnnotator = () => {
  ...
  const rows = data.annotators.map((annotator) => (
    <tr key={annotator.email}>
      ...
      {annotator.isActive === true ? (
        <td>
          <Button color="green" onClick={() => openModal(annotator.isActive)}>
            Active
          </Button>
        </td>
      ) : (
        <td>
          <Button color="red" onClick={() => openModal(annotator.isActive)}>
            Inactive
          </Button>
        </td>
      )}
  ...
  const openModal = (isActive: Boolean) =>
    openConfirmModal({
      ...
      onConfirm: () => handleConfirm(isActive),
    });
